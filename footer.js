// Função que injeta o estilo e o HTML do rodapé original (Adaptado para o projeto de currículos)
function carregarRodape() {
    
    // O endereço do seu site oficial para onde todos os links devem apontar
    const sitePrincipal = 'https://www.resolvefacil.online/';

    // Lógica do botão Voltar: Tenta voltar no histórico ou força a ida para a loja oficial
    ...
    // 1. GARANTIR QUE OS ÍCONES CARREGUEM (FontAwesome)
    ...
    // 2. ESTILOS CSS (EXATAMENTE IGUAIS AO SEU ORIGINAL)
    const styleHTML = `
        <style>
            ...
            /* --- ESTILOS DO RODAPÉ --- */
            .rf-footer-wrapper {
                background-color: var(--primary-bg, #003459);
                color: var(--text-color, #f2f2ed);
                margin-top: 40px;
                border-top: 4px solid var(--highlight, #ff6600);
                font-family: Arial, sans-serif;
            }
            .rf-footer-container {
                max-width: 1000px;
            ...
            .rf-footer-col h4 {
                ...
                font-size: 1.2em;
                margin-bottom: 20px;
                border-bottom: 2px solid var(--secondary-bg, #3e5a6a);
                padding-bottom: 10px;
            ...
            .rf-social-flex a:hover {
                background-color: var(--highlight, #ff6600);
            ...
            .rf-scroll-top-btn {
                ...
                background-color: var(--secondary-bg, #3e5a6a);
                color: white;
                border: 2px solid var(--highlight, #ff6600);
                padding: 10px 25px;
                border-radius: 25px;
                cursor: pointer;
                font-weight: bold;
            ...
            @media (max-width: 768px) {
                .rf-footer-container { grid-template-columns: 1fr; text-align: center; }
                .rf-footer-col h4 { display: inline-block; }
                .rf-contact-list li { justify-content: center; }
                .rf-footer-logo { margin: 0 auto 15px auto; }
            }
        </style>
    `;

    // 3. O CÓDIGO HTML (USANDO LINKS ABSOLUTOS PARA O SITE PRINCIPAL)
    const footerHTML = `
    ...
    if (container) {
        ...
        // Ativa o botão de voltar ao topo
