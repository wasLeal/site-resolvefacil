// Arquivo: /netlify/functions/notificacao-pagamento-background.js (VERSÃO DE PRODUÇÃO DEFINITIVA)
...
async function processarPagamento(event) {
    ...
    try {
        ...
        if (body.type === 'payment') {
            ...
            console.log(`Evento 'payment' VÁLIDO recebido. Processando Pagamento ID: ${paymentId}`);
            ...
            console.log("Detalhes do pagamento recebidos. Status:", payment.status);
            ...
            if (payment.status === 'approved') {
                ...
                console.log(`Pagamento ${paymentId} não está com status 'approved'. Status atual: ${payment.status}.`);
            }
        } else {
            console.log(`Webhook de tipo '${body.type}' recebido e IGNORADO PROPOSITALMENTE.`);
        }
    } catch (error) {
...
exports.handler = async function(event) {
    ...
    // Para Background Functions, a resposta é enviada antes do 'processarPagamento' terminar.
    return {
        statusCode: 200,
        body: 'Webhook recebido e aceito para processamento em segundo plano.'
    };
};